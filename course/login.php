<?php
	...
	if(!empty($user_name) && !empty($password) && !empty($kid) )
	{
		$pass = '111111';
		$strSql = "SELECT a.uid FROM commonuser AS a WHERE a.user_name = '".$user_name."' 
					AND a.password = '".$password."' AND a.uid IN (SELECT b.uid FROM course_user AS b 
									WHERE b.kid = '".$kid."' );";
		...
		if($count == 1)
		{
			if ( $psd != $pass )
			{
				...
				{
					$_SESSION['kuid'] = $oDD->GetValue($strSql, MYSQLI_NUM, 0);
